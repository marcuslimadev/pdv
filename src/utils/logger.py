...
class Logger:
    ...
    def _setup_logger(self):
        ...
        console_handler.setLevel(level)  # Console respeita config
    ...
    def log_venda(numero_venda: str, usuario: str, total: float):
        """Registra uma venda."""
        logger = Logger()
        logger.info(f"VENDA: {numero_venda} | Usuário: {usuario} | Total: R$ {total:.2f}".replace('.', ','))
    
    @staticmethod
    def log_pagamento(forma_pagamento: str, valor: float, status: str):
        """Registra um pagamento."""
        logger = Logger()
        logger.info(f"PAGAMENTO: {forma_pagamento} | Valor: R$ {valor:.2f} | Status: {status}".replace('.', ','))
    
    @staticmethod
    def log_abertura_caixa(usuario: str, valor_abertura: float):
        """Registra abertura de caixa."""
        logger = Logger()
        logger.info(f"ABERTURA DE CAIXA: Usuário: {usuario} | Valor: R$ {valor_abertura:.2f}".replace('.', ','))
    
    @staticmethod
    def log_fechamento_caixa(usuario: str, valor_fechamento: float):
        """Registra fechamento de caixa."""
        logger = Logger()
        logger.info(f"FECHAMENTO DE CAIXA: Usuário: {usuario} | Valor: R$ {valor_fechamento:.2f}".replace('.', ','))
